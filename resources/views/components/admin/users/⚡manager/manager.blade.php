<div>
    <x-common.component-card title="Kelola Pengguna" desc="Akun admin, dengan level akses Administrator, Admin, atau Operator.">
        <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
            <input type="text" wire:model.live.debounce.300ms="search" placeholder="Cari nama / username..."
                        ...
                        <tr wire:key="user-{{ $user->id }}">
                            ...
                            <td class="px-5 py-3 text-sm">
                                ...
                                <x-ui.badge :color="match ($peran) {
                                    ...
                                    'admin' => 'primary',
                                    default => 'light',
                                }">{{ $this->rolePilihan[$peran] ?? $peran }}</x-ui.badge>
                            </td>
                            <td class="px-5 py-3 text-right text-sm">
                                <x-common.table-actions>
                                    <x-common.dropdown-item wire:click="edit({{ $user->id }})">Ubah</x-common.dropdown-item>
                                    <x-common.dropdown-item danger wire:click="delete({{ $user->id }})" wire:confirm="Yakin ingin menghapus pengguna ini?">Hapus</x-common.dropdown-item>
                                </x-common.table-actions>
                            </td>
                        </tr>
                    @empty
        ...
        <form wire:submit="save" class="max-h-[85vh] overflow-y-auto p-6">
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <x-form.select wire:model="role" placeholder="-- Pilih Role --">
                        @foreach ($this->rolePilihan as $kunci => $label)
                            <option value="{{ $kunci }}">{{ $label }}</option>
                        @endforeach
                    </x-form.select>
                    @error('role') <p class="mt-1 text-sm text-error-500">{{ $message }}</p> @enderror
                </div>
