{{--
    Full-page PBSF backdrop: navy→teal gradient, drifting Gold/Biru/Hijau
    Hujan orbs, a faded 64px grid and slanted teal "rain". Fixed to the
    viewport so it stays put while the page scrolls. Rain drop positions/
    timings are derived from the index (not random) so the markup is stable
    between requests.

    `calm` is for the peserta portal (components/layouts/cbt.blade.php):
    dimmer orbs and no rain, so nothing competes with the exam itself.
--}}
@props(['calm' => false])
...
<div aria-hidden="true" class="pointer-events-none fixed inset-0 overflow-hidden bg-pbsf-ink">
    ...
    {{-- Orbs --}}
    <div @class(['absolute -top-32 -left-24 size-[520px] animate-pbsf-orb-a rounded-full blur-[110px] motion-reduce:animate-none', 'bg-pbsf-blue/30' => ! $calm, 'bg-pbsf-blue/20' => $calm])></div>
    <div @class(['absolute top-1/4 right-[-160px] size-[460px] animate-pbsf-orb-b rounded-full blur-[110px] motion-reduce:animate-none', 'bg-pbsf-gold/20' => ! $calm, 'bg-pbsf-gold/10' => $calm])></div>
    <div @class(['absolute -bottom-48 left-1/3 size-[480px] animate-pbsf-orb-c rounded-full blur-[110px] motion-reduce:animate-none', 'bg-pbsf-rain/25' => ! $calm, 'bg-pbsf-rain/15' => $calm])></div>

    {{-- Hijau Hujan: slanted teal streaks --}}
    @unless ($calm)
        <div class="absolute -inset-x-1/4 inset-y-0 rotate-[14deg] motion-reduce:hidden">
            @for ($i = 0; $i < 32; $i++)
